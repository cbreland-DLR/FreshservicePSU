...
Describe 'Test-FsuConfiguration: optional properties and derived BaseUri' {

    BeforeEach {
        $script:config = New-FsuValidConfigHashtable
    }

    It 'accepts a minimal config with only the required properties' {
        $minimal = @{
            SchemaVersion = '1.0'
            Stage = 'Prod'
            Tenant = 'acme'
        ...
        $result = Test-FsuConfiguration -Config $minimal
        $result.PSObject.TypeNames | Should -Contain 'Freshservice.Configuration'
        $result.Tenant | Should -Be 'acme'
    }

    It 'derives BaseUri from Tenant when BaseUri is not supplied' {
        $config.Remove('BaseUri')
        $result = Test-FsuConfiguration -Config $config
        $result.BaseUri | Should -Be 'https://acme.freshservice.com/api/v2/'
    }

    It 'uses a supplied BaseUri override as-is when it is valid' {
        $config.BaseUri = 'https://acme.freshservice.com/api/v2/'
        $result = Test-FsuConfiguration -Config $config
        $result.BaseUri | Should -Be 'https://acme.freshservice.com/api/v2/'
    }

    It 'treats Authentication as optional and returns $null when it is absent' {
        $config.Remove('Authentication')
        $result = Test-FsuConfiguration -Config $config
        $result.Authentication | Should -BeNullOrEmpty
    }

    It 'treats Users as optional and returns an empty map when it is absent' {
        $config.Remove('Users')
        $result = Test-FsuConfiguration -Config $config
        @($result.Users.Keys).Count | Should -Be 0
    }

    It 'still validates Authentication when it is present but incomplete' {
        $config.Authentication.Remove('TrustedIssuers')
        { Test-FsuConfiguration -Config $config } | Should -Throw '*Authentication*'
    }

    It 'rejects a Tenant that is not a single subdomain label' -ForEach @(
        @{ Tenant = 'acme.freshservice.com' }
        @{ Tenant = 'https://acme' }
        @{ Tenant = 'acme/../evil' }
        @{ Tenant = 'acme.attacker.com' }
    ) {
        $config.Tenant = $Tenant
        { Test-FsuConfiguration -Config $config } | Should -Throw '*Tenant*'
    }
}
