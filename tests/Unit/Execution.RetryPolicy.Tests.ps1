...
BeforeAll {
    ...
    . (Join-Path $repoRoot '0.1.6/Private/Execution/Get-FsuRetryDecision.ps1')
...
Describe 'New-FsuRetryPolicy: budgets are configuration, not parameters' {
    ...
    It 'defaults the noninteractive budget to 180 seconds so a full rate-limit window fits' {
        # Freshservice windows are minute-level, so Retry-After can approach
        # 60s. 180 leaves room for the wait plus the retried request.
        (New-FsuRetryPolicy -Mode Noninteractive).TotalBudgetSeconds | Should -Be 180.0
    }
