...
.SYNOPSIS
    ...
    0.1.6/, per ARCHITECTURE.md §12/§13 and IMPLEMENTATION_PLAN.md
    Phase 2.
#>

BeforeDiscovery {
    $script:repoRoot = Split-Path -Parent $PSScriptRoot | Split-Path -Parent
    $script:moduleRoot = Join-Path $repoRoot '0.1.6'
...
Describe 'Forbidden legacy HTTP/parsing APIs (ARCHITECTURE.md §11/§12)' {
    ...
    Context 'No file under 0.1.6/ uses UseBasicParsing, ServicePointManager, or System.Web' -ForEach $(if (@($apiTextViolations).Count) { $apiTextViolations } else { @(@{ File = ''; Pattern = ''; EmptyForEach = $true }) }) {
        It "<File> does not reference '<Pattern>'" -Skip:([bool](Get-Variable -Name EmptyForEach -ValueOnly -ErrorAction SilentlyContinue)) {
...
Describe 'Direct HTTP execution confined to Private/Http (ARCHITECTURE.md §11)' {
    ...
    Context 'No file outside Private/Http calls Invoke-RestMethod, Invoke-WebRequest, or HttpClient directly' -ForEach $(if (@($publicHttpViolations).Count) { $publicHttpViolations } else { @(@{ File = ''; Command = ''; Line = 0; EmptyForEach = $true }) }) {
        It "<File>:<Line> does not call '<Command>'" -Skip:([bool](Get-Variable -Name EmptyForEach -ValueOnly -ErrorAction SilentlyContinue)) {
...
Describe 'No process-global credential/context/connection state (ARCHITECTURE.md §13)' {
    ...
    Context 'No Global: or Script: scoped credential/context/connection variable' -ForEach $(if (@($globalStateViolations).Count) { $globalStateViolations } else { @(@{ File = ''; Variable = ''; Line = 0; EmptyForEach = $true }) }) {
        It "<File>:<Line> does not define '<Variable>' at Global/Script scope" -Skip:([bool](Get-Variable -Name EmptyForEach -ValueOnly -ErrorAction SilentlyContinue)) {
