...
Describe 'Public command naming (ARCHITECTURE.md §12)' {
    ...
    Context 'No Public/ function name contains Fsu' -ForEach $(if (@($publicFunctions).Count) { $publicFunctions } else { @(@{ File = ''; Name = ''; EmptyForEach = $true }) }) {
        It "'<Name>' in <File> does not contain 'Fsu'" -Skip:([bool](Get-Variable -Name EmptyForEach -ValueOnly -ErrorAction SilentlyContinue)) {
...
Describe 'Manifest export shape (ARCHITECTURE.md §12)' {
    ...
    It 'FunctionsToExport is a subset of the 23-command SUPPORTED_COMMANDS.md inventory' {
