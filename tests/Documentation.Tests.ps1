BeforeAll {
    ...
    function Get-DocumentedCommand {
        ...
        foreach ($line in Get-Content -LiteralPath $Path) {
            ...
            if ($inSection -and $line -match '^\|(?: [^|]+ \|)? `(?<Command>(?:Add|Get|New|Remove|Search|Set)-FreshService[^` ]+)` \|') {
                $Matches.Command
            }
        }
    }
...
Describe 'Authoritative documentation consistency' {
    It 'defines exactly 23 supported commands' {
        $supportedCommands.Count | Should -Be 23
    }
    ...
    It 'marks the checked-in implementation as transitional' {
        ...
        $readme | Should -Match 'not yet a packaged'
    }
