...
$moduleRoot = Join-Path -Path $PSScriptRoot -ChildPath '0.1.6'
...
function Test-PowerShellSyntax {
    ...
    if (-not $Path) {
        $Path = @($moduleRoot, (Join-Path $PSScriptRoot 'tests'), (Join-Path $PSScriptRoot 'examples'), $PSCommandPath)
    }
...
function Invoke-FormatCheck {
    ...
    if (-not $Path) {
        $Path = @($moduleRoot, (Join-Path $PSScriptRoot 'tests'), (Join-Path $PSScriptRoot 'examples'))
    }
...
"@ | Set-Content -LiteralPath $tempScriptPath -Encoding utf8NoBOM
    ...
    $expectedExportCount = @((Import-PowerShellDataFile -Path $manifestPath).FunctionsToExport).Count
    if ([int]$result.CommandCount -ne $expectedExportCount) {
        throw "Clean import exports $($result.CommandCount) command(s); expected $expectedExportCount from FunctionsToExport."
    }
