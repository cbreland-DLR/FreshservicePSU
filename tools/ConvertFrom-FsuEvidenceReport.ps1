...
function Assert-FsuEvidenceSchema {
    ...
    $psuAllowed = @(
        ...
        'ClaimSummary', 'Claims', 'SharedState', 'ComparisonFingerprintsEnabled',
        'SanitizationNote'
    )
    $freshserviceAllowed = @(
...
foreach ($reportPath in $Path) {
    ...
    if ($kind -eq 'PSU') {
        ...
        $sharedState = Get-FsuPropertyValue $report 'SharedState'
        if ($sharedState -and (Get-FsuPropertyValue $sharedState 'Ran')) {
            foreach ($field in @(
                    'Role', 'ProbeHash', 'CacheCmdletsAvailable', 'MutexAcquired',
                    'MutexWaitMilliseconds', 'CacheVisibleBeforeMutex',
                    'CounterBefore', 'CounterAfter', 'ObservedInitializerState',
                    'CacheRoundTripSucceeded', 'CachePersistedAfterRestart',
                    'CleanupSucceeded', 'CandidateWorkerProcessCount', 'ErrorType'
                )) {
                $lines.Add((ConvertTo-FsuMarkdownRow -Field "Shared state: $field" -Value (Get-FsuPropertyValue $sharedState $field)))
            }
        }
    } else {
