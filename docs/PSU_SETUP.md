...
- **Status:** Accepted deployment contract; the 23-command surface is implemented. Packaging and live identity evidence are still required before production cutover.
...
completed FreshservicePSU module. Consumer examples that call the public
commands are in `examples/`. Do not deploy the development branch as a
`1.0.0` production artifact until Phase 8 packaging and operator
...
environment. `SchemaVersion`, `Stage`, `Tenant`, `DefaultSecret`, and
`AllowSystemFallback` are required; `BaseUri`, `Authentication`, and `Users`
are optional (a present `BaseUri` still must pass the tenant-bound
validation). Unknown properties, unsupported schema versions, unsafe base
URIs, and duplicate identity mappings fail closed.
...
6. A bounded read returns typed output and one terminal audit event.
7. `-WhatIf` on a mutation sends no request and emits no success event.
...
| Freshservice returns `429` | Expected under load: the module waits `Retry-After` and retries within the budget. Persistent `429`s mean too much concurrency — stagger schedules and lower PSU job concurrency. Interactive callers surface the `429` rather than waiting out a minute-long window |
| Partial-results error | Discard or explicitly reconcile emitted records; do not mark the report run complete |

See [ARCHITECTURE.md](ARCHITECTURE.md) for the security and runtime contracts.
