...
- **Command inventory and removal record:** [COMMAND_PRUNE_LIST.md](COMMAND_PRUNE_LIST.md)
...
Supporting reads for form values and core report dimensions — `Get-FreshServiceTicketField`, `Get-FreshServiceAgentGroup`, `Get-FreshServiceDepartment`, `Get-FreshServiceLocation`, and `Get-FreshServiceAgent` — are the cache-eligible reference reads in §11 and need no per-user attribution. The remaining reporting reads (`Get-FreshServiceRequester`, `Get-FreshServiceRequestedItem`, `Get-FreshServiceTask`, `Get-FreshServiceRequestApproval`, `Search-FreshServiceApproval`, `Get-FreshServiceSLAPolicy`, `Get-FreshServiceBusinessHour`) are not implicitly cacheable: the request pipeline applies the caller's authorization and the endpoint-specific pagination and record limits. Ticket creation still supplies the requester by email; agent and requester reads exist for reporting, not as prerequisites for creating a ticket.
...
| Rate limits | Reactive only: send the request, honor `Retry-After` on a `429`, and retry within the budget. Rate headers are telemetry and gate nothing. |
...
| Command scope | Export exactly the accepted 23 commands; CSAT is conditional and time entries are excluded. |
| Asset scope | Asset commands are reusable PSU capabilities. They update ordinary properties and can soft-delete an asset. They do not implement external synchronization, create, restore, permanent delete, or move. |
...
@{
    ...
    BaseUri             = 'https://acme.freshservice.com/api/v2/'  # optional - derived from Tenant when omitted
    DefaultSecret       = 'FreshService.System.Prod'  # required
    AllowSystemFallback = $true                 # required
...
Of these, `SchemaVersion`, `Stage`, `Tenant`, `DefaultSecret`, and
`AllowSystemFallback` are required; `BaseUri`, `Authentication`, and `Users`
are optional (a present `BaseUri` still must pass the tenant-bound validation
below). `Authentication` accepts only `AllowedTypes`, `TrustedTenants`, and
...
Enable OIDC alongside the existing SAML provider (same Entra tenant, unchanged
PSU access policies), confirm the claims via PSU's claim-information view
without recording tokens, and verify in development PSU that SAML and OIDC
produce the same canonical `PrincipalId` — and therefore select the same vault
entry — for each test user across interactive, app-token, schedule,
unknown-identity, and reused-runspace cases. Only then enable OIDC for normal
use. Removing SAML is an independent deployment choice and is not required for
...
Per-user keys are the committed mechanism, and a `user_id` field never impersonates the caller or substitutes for credential selection. This is a **decision subject to one open gate** — Q7 in [OPEN_QUESTIONS.md](OPEN_QUESTIONS.md): the sandbox spike may show that `user_id` genuinely controls note authorship, which would satisfy the same requirement with one system key and no registry.
...
This is a rule rather than a preference: PSU runs many modules in shared sessions, so a leaked generic helper name can shadow or be shadowed across modules, and the context/credential helpers are exactly the ones §13 requires never to escape the module — a name that cannot be mistaken for a supported command makes an accidental export visible on sight.

`PSTypeName` values are unaffected. They remain `FreshservicePSU.<Resource>` because they are output contracts rather than commands.

### Naming applies to unimplemented endpoints too

`FRESHSERVICE_API_V2_ENDPOINT_COMPARISON.csv` records per-family legacy coverage and Phase 0's explicit Add/Skip/Conditional decision for every row (see `API_V2_COVERAGE_MATRIX.md` for scope and method). Those rows describe *legacy endpoint families*; they are unrelated to the 23-command target surface in `SUPPORTED_COMMANDS.md`, so the naming rules are mostly a rule for *new* commands, not a rename exercise. The coverage review itself demonstrated why: capabilities hidden behind mode switches (`Remove-FreshServiceAsset -delete_forever`, `Remove-FreshServiceAgent -Forget`, `Set-FreshServiceAgent -convert_to_requester`) were invisible to the audit and got marked missing. Once each endpoint has exactly one command, coverage is mechanically verifiable by name.

This section fixes the name each gap will take **if** it is ever added, independent of the CSV's decision column, so the decision and the naming are not relitigated together:

| Gap | Naming |
| --- | --- |
| Ticket and change approvals, approval groups, global approvals | `Get`/`New`/`Set`/`Remove-FreshServiceApproval -Type Ticket\|Change`, `Send-FreshServiceApprovalReminder`, and `*-FreshServiceApprovalGroup`. `-Type` is admissible here because ticket and change approvals are parallel endpoints returning the same shape. |
| Solution category and folder restore, permanent delete | Extend the combined `Restore-*` `ValidateSet`; permanent delete becomes `Clear-FreshServiceSolutionCategory` / `Clear-FreshServiceSolutionFolder`. |
| Attachment deletion on tickets, conversations, projects, contracts | `Remove-FreshServiceTicketAttachment` and siblings — one command per parent resource, never a switch on the parent's `Remove-*`. |
| Major-incident promote/demote | Action verbs, not `Set-` switches. |
| Asset, purchase-order, software-user, contract move operations | `Move-FreshService*` if these move between something other than workspaces; workspace moves are out of scope. Confirm per endpoint before implementing. |
| Alerts, CABs, on-call schedules, shared fields | New families; each documented endpoint gets one command, and no sub-resource read is folded into its parent's `Get-`. |

Families the CSV marks **Skip** for scope reasons (workspace moves and `/releases` per `CLAUDE.md`; physical subtypes, devices, cloud resources; journeys and onboarding/offboarding; time entries and attachments) are out of scope rather than unnamed and get no reserved names.
...
failure audit event, and throws a terminating cancellation error. Cache entries
are released in `finally` paths. Contract tests cover cancellation before send,
during response, between pages, and while waiting to retry.
...
**Rate limiting is reactive.** The module sends the request, and honors a `429`
by waiting `Retry-After` and retrying within the retry budget. It keeps no
budget of its own and refuses no request in advance. Read total, remaining,
current-request cost, `Retry-After`, and API-version headers from responses and
surface them as telemetry on the response envelope; nothing gates on them.
Credential selection affects attribution, not throughput.

> **Decision reversed (supersedes the shared-limiter design).** An earlier
> revision specified a proactive reservation-based limiter (PSU server-cache
> backed, mutex-guarded, fail-closed); it is withdrawn and its `*-Fsu*`
> reservation helpers are removed. The module's own accounting could never be
> authoritative: the Freshservice limit is account-wide and shared with
> consumers the module cannot observe (other integrations, Workflow Automator,
> the vendor's own callers), so local counting bought consistency among PSU
> workers rather than correctness against the real budget. Accepted
> consequence: two PSU workers can each approach a sublimit and one absorbs a
> `429` — one refused request and a wait, safe to replay even for a
> non-idempotent method because a `429` means the request was not processed.
> The retry budget (see "Retry policy") decides whether a `429` can be waited
> out: noninteractive budgets fit a full minute-level window plus the retried
> request; interactive callers deliberately stay short so a PSU page surfaces
> a renderable error instead of hanging.

Documented per-minute limits (Enterprise plan, the tenant's plan):

| Action | Per-minute limit |
| --- | --- |
| Overall limit | 500 |
| List All Tickets / Assets / Agents / Requesters | 140 |
| View / Create / Update Ticket | 160 |
| Update Asset | 160 |

These numbers are documentation, not enforcement: nothing in the module counts against them. The 140-against-500 gap is why header-driven throttling would not have worked either — a paginated ticket query can be refused while `X-RateLimit-Remaining` still reports most of the account budget, because that header carries one number and cannot indicate which sublimit is being approached. The sublimits are recorded here so an operator reading a `429` knows which ceiling was likely hit, and so schedule staggering and PSU job-concurrency caps can be set with the real numbers in view.
...
`field` is preserved in the error record; it is what makes a 400 actionable and is exactly what the legacy transport discarded.
...
Closed decisions live in the §3 table (workspace, system-identity attribution, reactive rate limiting, output contract, consumer-owned persistence, reference cache, release mechanics). Two release details are recorded only here:
...
- The severe legacy defects identified in the pre-rewrite review remain absent by test.
